...
int render_add_object(render_t *r, asset_t *asset, matrix_3x3_t orientation,
		      vec_3_t offset)
{
...
void project_object(render_t *r, object_t *o)
{
	...
	for (int32_t j = 0; j < o->asset->v_count; j++) {
		...
		o->proj_v[j].position.y =
		    r->height - (v2.y * r->scaled_fov + centery);
	}
}
...
{
	timsort(obj->vf_sortable, obj->asset->f_count,
		sizeof(sortable_triangle), compFaces);
}
...
{				// no touching! (holy grale of projection, boris allowed only)
