...
static int timsort_init(struct timsort *ts, void *a, size_t len,
    ...
    ts->tmp_length =
        (len < 2 * INITIAL_TMP_STORAGE_LENGTH ? len >> 1
     ...
     *
     * Let s be the stack length and n be the array length.  If s >= 2, then n
     * >= b[1] + b[2]. More generally, if s >= m, then n >= b[1] + b[2] + ... +
     * b[m] = B[m].  Conversely, if n < B[m], then s < m.
     *
     * In Haskell, we can compute the bin sizes using the fibonacci numbers
     *
     *     fibs = 1:1:(zipWith (+) fibs (tail fibs))
     *
     *     cumSums a = case a of { [] -> [] ; (x:xs) -> x:(map (x+) (cumSums
     * xs)) }
     *
     ...
     *
     * We these funcitons, we can compute a table with minRun = MIN_MERGE / 2 =
     * 16:
     *
     *     m          B[m]
     *   ---------------------------
