...
int64_t millis()
{				// copy paste from stackoverflow (seems to be working)
	struct timespec now;
...
int32_t digest_events(event_handler_t *e)
{
	...
	do {
		...
		case SDL_WINDOWEVENT:
			...
			SDL_GetWindowSize(e->window, &e->render->width,
					  &e->render->height);
			e->render->scaled_fov =
			    fminf(e->render->width,
				  e->render->height) * e->render->fov_ratio;
			break;
		...
		case SDL_MOUSEMOTION:
			...
			e->x_angle =
			    (e->x_angle < -M_PI / 2) ? -M_PI / 2 : e->x_angle;
			e->x_angle =
			    (e->x_angle > M_PI / 2) ? M_PI / 2 : e->x_angle;
	...
	e->render->orientation =
	    lmul(matrix_3x3_rotation(vec_3(e->x_angle, 0, 0)),
	...
	matrix_3x3_t ro = lmul(matrix_3x3_rotation(vec_3(0, -e->y_angle, 0)),
			       matrix_3x3_rotation(vec_3(-e->x_angle, 0, 0))
	    );

	e->render->offset = ladd(e->render->offset, matrix_3x3_apply(ro, v)
	    );
