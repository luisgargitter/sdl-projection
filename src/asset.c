...
int obj_proc_line(asset_t *asset, char *line)
{
	...
	if (line[0] == 'v') {
		if (line[1] == ' ') {
			obj_scan_vec_3(asset->v_vector + asset->v_count,
				       line + 1);
			asset->v_count++;
		} else if (line[1] == 'n') {
...
int asset_load_obj(FILE *f, asset_t *a)
{
	...
	while (getline(&line, &n, f) > 0) {
		...
		if (a->v_count >= vc) {
			vc = vc * 2;
			a->v_vector =
			    realloc(a->v_vector, sizeof(*(a->v_vector)) * vc);
		} else if (a->n_count >= nc) {
			nc = nc * 2;
			a->n_vector =
			    realloc(a->n_vector, sizeof(*(a->n_vector)) * nc);
		} else if (a->t_count >= tc) {
			tc = tc * 2;
			a->t_vector =
			    realloc(a->t_vector, sizeof(*(a->t_vector)) * tc);
		} else if (a->f_count >= fc) {
			fc = fc * 2;
			a->f_vector =
			    realloc(a->f_vector, sizeof(*(a->f_vector)) * fc);
		}
